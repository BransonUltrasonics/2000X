/* $Header:   D:/SWLab/VMdb/2000Series/App/ABSOLUTE.C_V   1.33   10 Sep 2009 15:59:30   bkymer  $ */
/****************************************************************************/ 
/*                                                                          */
/*                                                                          */
...
/*--------------------------- MODULE DESCRIPTION ---------------------------*/
/*                                                                          */
/* Module name: Absolute Mode                                               */
/*                                                                          */
/* Filename:    Absolute.c                                                  */
/*                                                                          */
/* Function Name: SelectAbsoluteWeldSequence                                */
/*                                                                          */
...
/*     3       08-12-96   BEKymer    Change test condition for trigger      */
/*                                      delay test (0:not disabled)         */
/*     4       08-13-96   BEKymer    Add state SonicsOn3A                   */
...
/*    10.1.9   07-02-97   LAM        added states without an encoder        */
...
/*    1.13/14  09.15.97   JBerman    Added WeldSequence Bound Test                                                                    */
/*    1.15     11-13-97   LAM        moved stop data state                  */
/*    1.16     12-01-97   LAM        fixed stop data state                  */
/*    1.17     02-02-98   BEKymer    Do data collection for LEVEL_e or better*/
/*    1.18     04-27-98   BEKymer    SetCurrentSequenceType to WELD (like   */
/*                                   all other modes do!).                  */
/*    1.19     05-19-98   BEKymer    Add new state to check process alarms  */
/*    1.20     06-26-98   BEKymer    Allow for different trigger states     */
/*                                   depending if pretrigger is on or not.  */
/*                                   This means O/L will not be checked in  */
/*                                   trigger unless pretrigger is on.       */
/*    1.21     09-08-99   BEKymer    Modified sequence so R/T and pretrigger*/
...
/*    1.24     12-16-99   LAM        removed unused reference to profiling  */
/*    1.25     06-26-00   DJL        made global name changes from Preset.SeekFun to Preset.SeekFunFlag  */
/*                                   Changed MODEl to LEVEL                  */
/*    1.26     07-13-00   BEKymer/DJL  added actuator=aed to if statement.   */
/*    1.27     09-30-02   BEKymer    Added clear memory state at end of     */
...
/*    1.31     08-04-04   JOY        States StartData & StopData added in LEVEL_t */
...
/*--------------------------------------------------------------------------*/
   ...
   if ((RunningPreset.PreTrigFlag & BIT0) == BIT0) {                   /* If Pretrigger is on                       */
      if ( (CurrentSetup.Actuator == AEF) ||
           (CurrentSetup.Actuator == AED) ||                           /* and we have an encoder (AED or AEF)       */
           (CurrentSetup.Actuator == MICRO) ) {                        /* or a MICRO                                */
...
/*------  Determine if amplitude stepping is needed  ------*/
