/* $Header:   D:/databases/VMdb/2000Series/App/ENERGY.C_V   1.36   Jan 03 2018 12:27:50   RCHAPA  $ */
/****************************************************************************/ 
/*                                                                          */
/*                                                                          */
...
/*     4       09-12-96   BEKymer    Add post weld seek state               */
/*     5       09-12-96   BEKymer    Don't reset state machine index        */
/*     6       12-10-96   BEKymer    Add WaitUlsUp state before PreReady    */
/*     7       03-27-97   LAM        chged seq to send final data last      */
/*     8       04-07-97   LAM        deleted upstroke state                 */
/*     9.1.10  07-02-97   LAM        added states without an encoder        */
/*                                   deleted getcurrentmodel#               */
/*     9.1.11  07-07-97   LAM        deleted psos_tbl.h                     */
/*    10.1.12  07-30-97   BEKymer    Added StartData & StopData states      */
/*    1.13     08-26-97   LAM        deleted start datacollection           */
...
/*    1.17     05-19-98   BEKymer    Add new state to check process alarms  */
/*    1.18     06-26-98   BEKymer    Allow for different trigger states     */
/*                                   depending if pretrigger is on or not.  */
/*                                   This means O/L will not be checked in  */
/*                                   trigger unless pretrigger is on.       */
/*    1.19     09-08-99   BEKymer    Modified sequence so R/T and pretrigger*/
/*                                   are in order by distance.              */
/*    1.20     09-10-99   BEKymer    Modify sequence again so R/T and       */
/*                                   pretrigger are in order.               */
/*    1.21     09-10-99   BEKymer    Modified no R/T & Pretrigger seq.      */
/*    1.22     12-16-99   LAM        removed unused reference to profiling  */
/*    1.23     06-26-00   DJL        made global name changes from Preset.SeekFun to Preset.SeekFunFlag */
/*    1.24     07-11-00   BEKymer/DJL added actuator=aed in if statements.  */
/*    1.25     07-13-00   BEK/DJL    removed all tabs                       */
/*    1.26     05-02-01   BEKymer    Added SelectEnergyWeldSequenceHH to    */
/*                                    support handheld and all single start */
/*                                    weld cycles                           */
...
/*                                    change is lost.                       */
/*    1.28     09-11-03   BEKymer    Add energy braking state, remove Mem   */
/*                                    ClearST                               */
...
/*    1.32     01-05-06   LAM         Add 2nd PB state when HH and presets on*/
/*    1.33     09-02-08   NHAhmed    CleanUp for multi-language support.    */
/*    1.34     09-10-09   BEkymer    Add new define MICRO where needed      */
/*    1.35	   01-03-18   RChapa	 Removed call for DUPS State            */
/*                                   to update DUPS parameters              */
/*----------------------------- DESCRIPTION --------------------------------*/
...
/*------  Determine if stepping is needed  ------*/
   ...
   if ( (CurrentSetup.Actuator == AEF) ||
        (CurrentSetup.Actuator == AED) ||
        (CurrentSetup.Actuator == MICRO) ) {
      WeldSequence[i++] = &SonicsOn4DST;  /* Amplitude step is off  */
   }
   else{
      WeldSequence[i++] = &SonicsOn4ST;  /* Amplitude step is off  */
   }
...
/***************************************************************************/
{
   ...
   if (CurrentSetup.ExtPresets == TRUE) {
      WeldSequence[i++] = &PBStateHHST;   /* Need an extra state when both HH and and external presets are selected  */
   }   
   WeldSequence[i++] = &StartData;
