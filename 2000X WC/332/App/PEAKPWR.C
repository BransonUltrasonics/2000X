/* $Header:   D:/databases/VMdb/2000Series/App/PEAKPWR.C_V   1.35   Jan 03 2018 11:37:18   RCHAPA  $ */
/****************************************************************************/ 
/*                                                                          */
/*                                                                          */
...
/*     4       08-13-96   BEKymer    Add state SonicsOn3C                   */
...
/*    10.1.10  07-02-97   LAM        added states without an encoder        */
/*                                   deleted getcurrentmodel#               */
/*    10.1.11  07-07-97   LAM        deleted psos_tbl.h                     */
/*    10.1.12  07-30-97   BEKymer    Added StartData & StopData states      */
/*    1.13     08-26-97   LAM        deleted start datacollection           */
/*    1.14     09-08-97   BEKymer    Check number of states against size of */
/*                                   WeldSequence array                     */
...
/*    1.18     05-19-98   BEKymer    Add new state to check process alarms  */
/*    1.19     06-26-98   BEKymer    Allow for different trigger states     */
/*                                   depending if pretrigger is on or not.  */
/*                                   This means O/L will not be checked in  */
/*                                   trigger unless pretrigger is on.       */
/*    1.20     09-08-99   BEKymer    Modified sequence so R/T and pretrigger*/
/*                                   are in order by distance.              */
/*    1.21     09-10-99   BEKymer    Modify sequence again so R/T and       */
/*                                   pretrigger are in order.               */
/*    1.22     09-10-99   BEKymer    Modified no R/T & Pretrigger seq.      */
/*    1.23     12-16-99   LAM        removed unused reference to profiling  */
/*    1.24     06-26-00   DJL        made global name changes from Preset.SeekFun to Preset.SeekFunFlag */
/*                                  changed model to level                 */
/*    1.25     07-11-00  BEKymer/DJL added actuator=aed to if statement.   */
/*    1.26     05-02-01  BEKymer     Added SelectPeakPowerWeldSequenceHH    */
...
/*------  Determine if stepping is needed  ------*/
   ...
   if ( (CurrentSetup.Actuator == AEF) ||
        (CurrentSetup.Actuator == AED) ||
        (CurrentSetup.Actuator == MICRO) ) {                        /* and we have an encoder (AED, AEF or MICRO)*/
      WeldSequence[i++] = &SonicsOn7DST;  /* Amplitude step is off  */
   }
   else{
      WeldSequence[i++] = &SonicsOn7ST;   /* Amplitude step is off  */
   }
